...
const Card = (p:Props) => {
  ...
  return (
    <div className='card w-96 bg-gray-400/25 p-5 m-5 rounded-3xl backdrop-blur-3xl'>
        <a href="#" className='text-2xl font-semibold block mb-1'>{p.data.domainAddress}</a> <br />
        <span className='border-b-2 border-white text-3xl font-medium'>N/A</span>
        <p className='my-4 text-xl font-normal'>{description}</p>
        <div className='flex text-green-600 text-2xl font-medium'>
        <button className='bg-white m-1 py-1 px-3 rounded-3xl cursor-auto'>{p.data.siteSafety}</button>
        <button className='bg-white m-1 py-1 px-3 rounded-3xl cursor-auto'>{p.data.siteType}</button>
        </div>
    </div>
  )
}
