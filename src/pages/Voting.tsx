...
import AllReportsContainer from '../Components/VotingContainer';
...
const AllReport = () => {
  ...
  useEffect(() => {
    ...
    const filteredData = payload.filter(item => item.id === id);
    setprojectsData(filteredData);
          console.log("my reports", payload)
        }
      } catch (error) {
...

{
  !buttonset && (
    <>
    <h1 className="text-start lg:text-4xl md:text-4xl text-xl font-bold leading-none tracking-normal text-gray-100 md:text-3xl md:tracking-tight lg:flex md:flex">
                    <button onClick={() => setVerifiedproj(false)} className={`text-white lg:ml-20 md:ml-20 ml-10 mt-10`}>Participate in Report Voting</button>
                  </h1>
...
<div className="justify-end flex pr-20">
  <Link to="/allreports">
<button style={button}  className='py-2 px-10 rounded-lg'>
  Vote Again
  </button>
  </Link>
</div>
