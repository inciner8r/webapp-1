...
const Submit_review = () => {
  ...
  const [siteCat, setSiteCat] = useState<string>('');
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.5 }}>
      ...
      <div className="modal">
        ...
        >
          ...
          <h3 className="text-4xl font-bold mb-3 text-white">Share Your Insights</h3>
          {/* <p className="mb-5 text-gray-300">Fill out the following form to submit your review!</p> */}
  
          <form onSubmit={(e) => { e.preventDefault(); handleSubmit(); }}>
            <div className="mb-4">
              <input type="text" id="title" className="w-full p-2 rounded shadow-md shadow-green-200/60  my-1 bg-black text-gray-300" placeholder='Website Name' value={title} onChange={(e) => setTitle(e.target.value)} required />
            </div>
  
            <div className="mb-4">
              <input type="url" id="websiteUrl" className="w-full p-2 rounded shadow-md shadow-green-200/60  my-1 bg-black text-gray-300" placeholder='URL' value={websiteUrl} onChange={(e) => setWebsiteUrl(e.target.value)} required />
            </div>
            ...
            <div className="mb-4">
              <select id="siteCat" className="w-full p-2 rounded shadow-md shadow-green-200/60  my-1 bg-black text-gray-300" value={siteCat} onChange={(e) => setSiteCat(e.target.value)} required>
                <option value="">Category</option>
                <option value="Genuine">Social Media</option>
              </select>
            </div> 
            ...
            <select id="siteType" className="w-full p-2 rounded shadow-md shadow-green-200/60  my-1 bg-black text-gray-300" value={siteSafety} onChange={(e) => setSiteSafety(e.target.value)} required>
              ...
              <option value="Common Website">Mostly Safe</option>
              <option value="Software">Adware Issues</option>
              <option value="Defi Project">Malware Threats</option>
              <option value="Wallet Address">Spyware Risks</option>
              <option value="Company">Phishing concerns</option>
            </select>
          </div>
